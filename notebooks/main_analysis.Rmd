...
nsd <- tribble(~ class, ~ total_species, 
               ...
               "Gastropoda", prod(c(40e3, 100e3))^(1/2), # https://ucmp.berkeley.edu/taxa/inverts/mollusca/gastropoda.php
...
nsd %>% 
  dt_dps %>% 
  select(phylum, species) %>% group_by(phylum)  
...
dnsp_class <- dt_dps %>% 
  filter(is.finite(log10_popsize), is.finite(log10_body_mass_pred)) %>%
  select(class, species) %>% group_by(class) %>% 
               summarize(nspecies=n())

dnsp_class %>%
  ggplot(aes(class, log10(frac), size=log10(total_species))) + geom_point() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
dnsp_phyla %>%
  ggplot(aes(phylum, log10(frac), size=log10(total_species))) + geom_point() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
baron_mine %>% left_join(dnsp_phyla, by='phylum')  %>% select(nspecies.x, nspecies.y)
  mutate(a=signif(total_carbon_mass / frac, 2)) %>% 
  select(-bd_total_biomass) %>%
  select(-total_log10_popsize, -nspecies.y) %>% as.data.frame
...
xd %>% filter(class == 'Mammalia') %>% arrange(desc(carbon_mass)) %>% 
  select(species, carbon_mass) %>% 
  as.data.frame()
...
Ok, now let's look at total biomass. Since I have drastically different phylum
represented here in my taxa sample than on earth, we need to reweight things.



```{r}

# using the Bar-On weights
# prop_carbon_mass is prop in my dataset
baron_mine <- baron_mine %>% 
              mutate(reweight = bd_prop_mass / prop_carbon_mass) %>%
              mutate(reweighted_total_carbon_mass = reweight * total_carbon_mass) 

total_sample_carbon_mass <- baron_mine %>% pull(reweighted_total_carbon_mass) %>% sum()

# total species here
n <- nrow(xd)
total_animal_species <- sum(nspecies)

print(paste0("fraction of animal species in this data set: ", 
             signif(n/total_animal_species, 2)))

# Bar-On et al. (2018) estimate is 2Gt C
# 1e12 kg = 1Gt
total_animal_cmass <- 2 

# if the species here were random
signif(total_animal_cmass * n / total_animal_species, 2)
signif(total_sample_carbon_mass, 2)

0.058 / total_animal_cmass

```

Note: initially it was my plan to see if we could get a sense of what fraction
of the total biomass this dataset contains (which is about 3%). This seems
unreasonably high, even after reweighting as above. The problem is that the
3.4e-5 fraction of animal species is obviously not a random sample based on
size / population, and includes humans, domesticated species, etc. -- this
probably is accentuated by the overabundance of chordates compared arthropods
here.
...
log10_popsize_seq <- seq(5, 18, length.out=100)
...
dml_full <- da_ml %>%
     ...
     mutate(Ne_N_RHH_BGS_strongsel_strongBGS = RHH_BGS(1, L=map_length, 
                                                       N=10^log10_popsize,
                                             B=Ne_N_HK95_strongBGS, gamma=dm_gamma, 
                                             J22=J22 * 10)) %>%
