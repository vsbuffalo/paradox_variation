...
So, marine species are over represented in my dataset.
...
# 1Gt = 1e12 kg
xd$carbon_mass <- (prop_carbon*(10^xd$pred_log10_body_mass) / 1e3 * # to kg
...
xd %>% filter(species == "Aptenodytes patagonicus") %>% select(species, log10_popsize)

xd %>% filter(genus == "Equus") %>% select(species, log10_popsize, log10_range)
