...
Based on the predictions of Mora et al. 2011
...
# 18-23% of human body is carbon; seems reasonable this holds
# for other animals

# only complete data
xd <- d %>% filter(is.finite(log10_body_mass))



# in Gt
prop_carbon <- signif(sqrt(0.18 * 0.23), 1)  # ~ 0.2
# 1Gt = 1e12 kg
xd$carbon_mass <- (prop_carbon*(10^xd$pred_log10_body_mass) / 1e3 * 10^xd$log10_popsize) / 1e12
...
baron_d <- tribble(~ phylum, ~ bd_mass,
           ...
           # not we need to reweight the original proportions as some phyla were
           # dropped
           mutate(prop_carbon_mass = prop_carbon_mass / sum(prop_carbon_mass))
           

ggplot(baron_d, aes(bd_prop_mass, prop_carbon_mass, color=phylum))  + 
...
baron_d <- baron_d %>% mutate(prop_bd_biomass = total_carbon_mass / bd_mass) 

baron_d %>%
  select(phylum, prop_bd_biomass)
...
total_sample_carbon_mass <- baron_d %>% pull(reweighted_total_carbon_mass) %>% sum()
