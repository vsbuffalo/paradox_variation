...
\citep{Kimura1964-ul,Malecot1948-zv,Wright1931-fl}. In particular, the
...
\citep[][p.  208]{Lewontin1974-jb}; this is known as Lewontin's ``Paradox
...
\begin{figure}
  \includegraphics[width=0.7\linewidth]{figures/Nc_figure.pdf}
  \caption{The distribution of approximate census population sizes estimated by this
   ...
   are available in \cite{Damuth1987-sg}.}{\includegraphics[]{figures/damuth.pdf}}\label{figsupp:sf1}
...
class.}{\includegraphics{figures/species_counts.pdf}}\label{figsupp:species-counts}
...
ranges shows the alpha hull method works well.}{\includegraphics[]{figures/iucn_redlist_ranges.pdf}}\label{figsupp:redlist-ranges}
...
\cite{Leffler2012-zj}.}{\includegraphics[]{figures/range_categories.pdf}}\label{figsupp:range-cats}
...
Stan.}{\includegraphics[]{figures/body_mass_length.pdf}}\label{figsupp:body-mass-length}
...
\FIGSUPP[figure-nc]{sf1}; \cite{Damuth1981-st,Damuth1987-sg}). Then, from
...
\begin{figure}
  ...
  \caption{A visualization of Lewontin's Paradox of Variation. Pairwise
    ...
    \mu \le 10^{-8}$.}{\includegraphics[width=\textwidth]{figures/diversity_popsize_linear.pdf}}\label{figsupp:popsize-linear}
...
phyla.}{\includegraphics[width=\textwidth]{figures/diversity_popsize_averages.pdf}}\label{figsupp:popsize-ave}
  ...
  model.}{\includegraphics[width=\textwidth]{figures/diversity_range_bodymass.pdf}}\label{figsupp:div-range-bodymass}
  ...
  means.}{\includegraphics{figures/diversity_leffler_ranges.pdf}}\label{figsupp:div-leffler}
  \end{fullwidth}
\end{figure}
...
\begin{figure}
  \begin{fullwidth}
  \includegraphics[width=\linewidth]{figures/diversity_pcm.pdf}
  ...
  gray line indicates zero.}{\includegraphics[width=\textwidth]{figures/div_popsize_post_phyla.pdf}}\label{figsupp:div-popsize-post-phyla}

  \figsupp{The ancestral continuous trait estimates for diversity and population size with species labels.}{\includegraphics[width=\textwidth]{figures/biconmap_diversity.pdf}}\label{figsupp:anc-species}
...
labels.}{\includegraphics[width=\textwidth]{figures/biconmap.pdf}}\label{figsupp:anc-maplen}
...
indicates phylum.}\label{fig:linksel}
  ...
  exceptionally large genome size (~30Gbp).}{\includegraphics[width=\textwidth]{figures/genome_size_popsize.pdf}}\label{figsupp:genome-size-Nc}
  ...
  \figsupp{The relationship between genome size and recombination map length.
    ...
    salamander (\emph{Ambystoma tigrinum}) was excluded because of its
  exceptionally large genome size (~30Gbp).}{\includegraphics[width=\textwidth]{figures/genome_size_maplength.pdf}}\label{figsupp:genome-size-maplen}
...
$\widetilde{N}_e$.}{\includegraphics[width=\textwidth]{figures/corbett_detig.pdf}}\label{figsupp:reductions}
...
\figsupp{Comparison of the \emph{Drosophila} sweep parameters used in this study with parameters from other studies.
    ...
    Supplementary Table S6).}{\includegraphics[width=\textwidth]{figures/linked_sel_params.pdf}}\label{figsupp:linked-sel-params}
...
\ref{figsupp:corbett_detig}A), and it is often thought that this is driven by
higher rates of adaptive substitutions in larger populations
\citep{Ohta1992-yi}, despite equivocal evidence \citep{Galtier2016-dq}.
...
map length, $L$, is known to correlate with body mass \citep{Burt1987-tq} and
thus varies inversely with population size. As this is a critical parameter
...
map lengths across species \citep{Stapley2017-fs,Corbett-Detig2015-gt}. I find
a significant non-linear relationship using phylogenetic mixed-effects models
(Figure \ref{fig:figure-3}A; see Methods and Materials:
\nameref{sec:methods-pcm}).  There is also a correlation between map length and
genome size (Supplementary Figure \ref{supfig:genome_size_maplength}) and
genome size and population size (Supplementary Figure
\ref{supfig:genome_size_popsize}). These findings are consistent with both the
hypothesis that non-adaptive processes increase genome size in small-$N_e$
species \citep{Lynch2003-bn} which in turn could increase map lengths, as well
as the hypothesis that map lengths are adaptively longer to more efficiently
select against deleterious alleles \citep{Roze2021-ya}. Overall, the negative
...
the rate of adaptive substitutions across species \citep{Galtier2016-dq}
and mutation rates \citep{Lynch2010-ki}. Furthermore, linked selection has
...
\ref{figsupp:corbett_detig}B). The high reductions in $\pi$ predicted here
...
assuming that $N_e = N_c$ (Supplementary Figure \ref{figsupp:corbett_detig}B).
...
\cite{Corbett-Detig2015-gt}, 96 are from \cite{Leffler2012-zj}, and 62
are from \cite{Romiguier2014-bp}. The Corbett-Detig et al. data is
...
\cite{Leffler2012-zj} were included to maximize the taxa in the study,
...
Supplementary Figure \ref{figsupp:damuth}). Using Stan
...
\texttt{rgbif} R package \citep{Chamberlain2014-up,Chamberlain2017-uz}.
...
packages \citep{South2017-db,Pebesma2018-bs}. For each taxon, I estimated
...
website, \citep{Yukilevich2012-vn,Yukilevich2017-jg}. To further validate
these range estimates, I have compared these to the qualitative range
descriptions \cite{Leffler2012-zj} (Supplementary Figure
\ref{figsupp:range-cats}) and compared my $\alpha$-shape method to a subset of
taxa with range estimates from IUCN Red List
(\cite{Chamberlain2020-tv,Iucn2020-ap}; Supplementary Figure
\ref{figsupp:redlist-ranges}). Each census population size
...
depicted in Supplementary Figure \ref{figsupp:species_counts}.  
...
convergence (Supplementary Figure \ref{figsupp:div_popsize_post_phyla}). 
...
(\texttt{phylo\_mm\_lognormal.stan}). Continuous trait maps (\FIG{pcm}A and
Supplementary Information Figure \ref{figsupp:biconmap}) were created using
...
\cite{Stapley2017-fs} and \cite{Corbett-Detig2015-gt}. The mathematical
...
\cite{Hu2013-pg}.  Following \cite{Elyashiv2016-vt}, only substitutions
...
\ref{figsupp:linked-sel-params}). Finally, I use $U_\text{Dmel} = 1.6$, from
\cite{Elyashiv2016-vt}. With these parameter estimates from \emph{D.
...
%   \figsupp{A version of Figure \ref{fig:figure-1} with diversity on a linear,
...
%   \figsupp{Diversity and approximate population size for 172 taxa, colored by
...
%   \figsupp{The relationship between diversity (differences per basepair) and
...
%   % figure 4-supplement 1
%   \figsupp{(A) The diversity data from \textcite{Corbett-Detig2015-gt} and the
